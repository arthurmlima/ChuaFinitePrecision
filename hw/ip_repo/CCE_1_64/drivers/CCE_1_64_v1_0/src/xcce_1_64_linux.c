...
typedef struct {
    ...
    XCce_1_64_uio_map maps[ MAX_UIO_MAPS ];
} XCce_1_64_uio_info;

/***************** Variable Definitions **************************************/
static XCce_1_64_uio_info uio_info;
...
static int uio_info_read_name(XCce_1_64_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCce_1_64_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCce_1_64_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCce_1_64_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCce_1_64_Initialize(XCce_1_64 *InstancePtr, const char* InstanceName) {
	XCce_1_64_uio_info *InfoPtr = &uio_info;
